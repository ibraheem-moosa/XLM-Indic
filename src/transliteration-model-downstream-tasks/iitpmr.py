...
def normalize_text(text):
    ...
    text = text.replace('\n', ' ').strip()
    return text
indic_glue_iitpmr_hi = load_dataset('indic_glue', 'iitp-mr.hi')
label_counter = Counter(indic_glue_iitpmr_hi['train']['label'])
...
num_classes = 3
class IITPMRHindiClassification(Dataset):
...
train_dataset = IITPMRHindiClassification(indic_glue_iitpmr_hi, tokenizer, 'train')
valid_dataset = IITPMRHindiClassification(indic_glue_iitpmr_hi, tokenizer, 'validation')
...
for seed in [101, 102, 103, 104, 105, 106, 107, 108, 109]:
    ...
    test_dataset = IITPMRHindiClassification(indic_glue_iitpmr_hi, tokenizer, 'test')
