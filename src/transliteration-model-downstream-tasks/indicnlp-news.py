...
indicnlp_news_train = pd.read_csv('../indicnlp-news-articles/{0}/{0}-train.csv'.format(lang), header=None, names=['label', 'text'])
indicnlp_news_valid = pd.read_csv('../indicnlp-news-articles/{0}/{0}-valid.csv'.format(lang), header=None, names=['label', 'text'])
indicnlp_news_test = pd.read_csv('../indicnlp-news-articles/{0}/{0}-test.csv'.format(lang), header=None, names=['label', 'text'])

labels = indicnlp_news_train.label.unique()
num_classes = len(labels)
labels_dict = dict((name, index) for index, name in enumerate(labels))

class IndicNLPNewsClassification(Dataset):
    
    def __init__(self, df, tokenizer, labels_dict):
        self.df = df
...
train_dataset = IndicNLPNewsClassification(indicnlp_news_train, tokenizer, labels_dict)
valid_dataset = IndicNLPNewsClassification(indicnlp_news_valid, tokenizer, labels_dict)
...
for seed in [101, 102, 103, 104, 105, 106, 107, 108, 109]:
    ...
    test_dataset = IndicNLPNewsClassification(indicnlp_news_test, tokenizer, labels_dict)
